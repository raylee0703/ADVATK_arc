...
def get_args():
    ...
    parser.add_option('--detector_path', dest='detector_path',type='string',
                      default='checkpoints/arc16.pth', help='detector_path')
...
def test(model, args):
    ...
    img_test = torch.zeros(1, 1, 40, 40)#.cuda()
    print(device_defense)
    torch.onnx.export(detector.model, img_test, "magnet.onnx", export_params=True, opset_version=12, do_constant_folding=True, input_names = ['input'], output_names = ['output'])
