...
from utils.magnet.dataset_arc import carstreamDataset
...
def get_args():
    ...
    parser.add_option('--data_path', dest='data_path',type='string',
                      default='../dataset/ivslab_train/JPEGImages/All/Gray', help='data path')
    ...
    parser.add_option('--width', dest='width', default=40, type='int',
                      help='image width')
    parser.add_option('--height', dest='height', default=40, type='int',
                      help='image height')
    parser.add_option('--batch_size', dest='batch_size', default=16, type='int',
                      help='batch size')
...
def train_net(model, args):
    ...
    if args.model_number == 1:
        model_path = os.path.join(model_folder, 'arc16.pth')
    ...
    for epoch in range(num_epochs):  # loop over the dataset multiple times
        ...
        for data in tqdm(train_loader, desc="Epoch %d progress: " % (epoch+1)):
            ...
            train_data = torch.from_numpy(slice_data_for_arc(np.array(data), args.width, args.height)).float()/255
        ...
        torch.save(model_state_dict, os.path.join(model_folder, 'arc16_q.pth'))
